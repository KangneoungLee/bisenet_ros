...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        ...
        image = torch.tensor(np.array(image.transpose((2, 0, 1)),dtype=np.float64))
...
class ColorJitter(object):
    ...
    def adj_saturation(self, image, rate):
        M = np.float32([
        ...
        shape = image.shape
        image = np.matmul(image.reshape(-1, 3), M).reshape(shape)/3
        image = np.clip(image, 0, 255).astype(np.uint8)
        image = np.float32(image)
        return image

    def adj_brightness(self, image, rate):
        table = np.array([
    ...
    def adj_contrast(self, image, rate):
        table = np.array([
...
class RandomResizedCrop(object):
    ...
    def __call__(self, image, label):
        ...
        im_h, im_w = [math.ceil(el * scale) for el in image.shape[:2]]
